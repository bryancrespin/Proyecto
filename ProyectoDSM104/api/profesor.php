...
switch ($method) {
    ...
    case 'POST':
        ...
        $stmt = $pdo->prepare("INSERT INTO profesores (nombre, apellido, edad, especialidad) VALUES (?, ?, ?, ?)");
        $stmt->execute([$data['nombre'], $data['apellido'], $data['edad'], $data['especialidad']]);
    ...
    case 'PUT':
        ...
        $stmt = $pdo->prepare("UPDATE profesores SET nombre = ?, apellido = ?, edad = ?, especialidad = ? WHERE id = ?");
        $stmt->execute([$data['nombre'], $data['apellido'], $data['edad'], $data['especialidad'], $data['id']]);
        $profesor = [
        'id' => $data['id'],
        'nombre' => $data['nombre'],
        'apellido' => $data['apellido'],
